...
local Settings = DarkWeb.Bluelib.Settings
local Flags = DarkWeb.Bluelib.Flags

local SetProps, SetChildren, InsertTheme, Create do
	InsertTheme = function(Instance, Type)
		table.insert(DarkWeb.Bluelib.Instances, {
			Instance = Instance,
			Type = Type
		})
		return Instance
	...
	local function Save(file)
		if readfile and isfile and isfile(file) then
			...
			if type(decode) == "table" then
				if rawget(decode, "UISize") then DarkWeb.Bluelib.Save["UISize"] = decode["UISize"] end
				if rawget(decode, "TabSize") then DarkWeb.Bluelib.Save["TabSize"] = decode["TabSize"] end
				if rawget(decode, "Theme") and VerifyTheme(decode["Theme"]) then DarkWeb.Bluelib.Save["Theme"] = decode["Theme"] end
			end
		end
	end
	
	pcall(Save, "DarkWeb.Blue library V5.json")
end
...
local Connections, Connection = {}, DarkWeb.Bluelib.Connection do
...
local function VerifyTheme(Theme)
	for name,_ in pairs(DarkWeb.Bluelib.Themes) do
		if name == Theme then
			return true
		end
	end
...
local Theme = DarkWeb.Bluelib.Themes[DarkWeb.Bluelib.Save.Theme]

local function AddEle(Name, Func)
	DarkWeb.Bluelib.Elements[Name] = Func
end

local function Make(Ele, Instance, props, ...)
	local Element = DarkWeb.Bluelib.Elements[Ele](Instance, props, ...)
	return Element
end
...
-- /////////// --
function DarkWeb.Bluelib:GetIcon(index)
...
function DarkWeb.Bluelib:SetTheme(NewTheme)
	if not VerifyTheme(NewTheme) then return end
	
	DarkWeb.Bluelib.Save.Theme = NewTheme
	SaveJson("DarkWeb.Blue library V5.json", DarkWeb.Bluelib.Save)
	Theme = DarkWeb.Bluelib.Themes[NewTheme]
	
	Comnection:FireConnection("ThemeChanged", NewTheme)
	table.foreach(DarkWeb.Bluelib.Instances, function(_,Val)
...
function DarkWeb.Bluelib:SetScale(NewScale)
	NewScale = ViewportSize.Y / math.clamp(NewScale, 300, 2000)
	UIScale, ScreenGui.Scale.Scale = NewScale, NewScale
end

function DarkWeb.Bluelib:MakeWindow(Configs)
	local WTitle = Configs[1] or Configs.Name or Configs.Title or "DarkWeb.Blue Library V5"
	local WMiniText = Configs[2] or Configs.SubTitle or "by : DarkWeb.Blue9999"
	...
	local UISizeX, UISizeY = unpack(DarkWeb.Bluelib.Save.UISize)
	...
	local MainScroll = InsertTheme(Create("ScrollingFrame", Components, {
		Size = UDim2.new(0, DarkWeb.Bluelib.Save.TabSize, 1, -TopBar.Size.Y.Offset),
	...
	ConnectSave(ControlSize1, function()
		if not Minimized then
			DarkWeb.Bluelib.Save.UISize = {MainFrame.Size.X.Offset, MainFrame.Size.Y.Offset}
			SaveJson("DarkWeb.Blue library V5.json", DarkWeb.Bluelib.Save)
		end
	end)
	
	ConnectSave(ControlSize2, function()
		DarkWeb.Bluelib.Save.TabSize = MainScroll.Size.X.Offset
		SaveJson("DarkWeb.Blue library V5.json", DarkWeb.Bluelib.Save)
	end)
	...
	function Window:SelectTab(TabSelect)
		if type(TabSelect) == "number" then
			DarkWeb.Bluelib.Tabs[TabSelect].func:Enable()
		else
			for _,Tab in pairs(DarkWeb.Bluelib.Tabs) do
	...
	function Window:MakeTab(paste, Configs)
		...
		TIcon = DarkWeb.Bluelib:GetIcon(TIcon)
		...
		local function Tabs()
			...
			table.foreach(DarkWeb.Bluelib.Tabs, function(_,Tab)
		...
		FirstTab = true
		local Tab = {}
		table.insert(DarkWeb.Bluelib.Tabs, {TabInfo = {Name = TName, Icon = TIcon}, func = Tab, Cont = Container})
		Tab.Cont = Container
		...
		function Tab:AddSection(Configs)
			...
			local Section = {}
			table.insert(DarkWeb.Bluelib.Options, {type = "Section", Name = SectionName, func = Section})
		...
		function Tab:AddDiscordInvite(Configs)
			...
			local TiktokInvite = {}
			function TiktokInvite:Destroy() InviteHolder:Destroy() end
			function TiktokInvite:Visible(...) Funcs:ToggleVisible(InviteHolder, ...) end
			return TiktokInvite
		end
		return Tab
	end
